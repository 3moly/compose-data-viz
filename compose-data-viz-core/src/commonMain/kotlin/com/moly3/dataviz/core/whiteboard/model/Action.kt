...
sealed class Action<ShapeType : Shape<Id>, Id> {
    fun <ShapeType : Shape<Id>> getOffsetMenu(shapes: List<Shape<Id>>): Offset =
        when (this) {
            is Connection -> {
                selectedConnection.getMenuCenter()
            }
    ...
    data class Connection<ShapeType : Shape<Id>, Id>(
        val selectedConnection: SelectedConnection<Id>,
    ) : Action<ShapeType, Id>()

    data class ShapeAction<ShapeType : Shape<Id>, Id>(
        val shape: ShapeType,
    ) : Action<ShapeType, Id>()

    data class DoubleClicked<ShapeType : Shape<Id>, Id>(
        val shape: ShapeType,
    ) : Action<ShapeType, Id>()
}
